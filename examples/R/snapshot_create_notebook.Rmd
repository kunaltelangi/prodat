---
title: "Log your experiments in R with prodat"
author: "Nick Walsh"
output:
  html_document:
    df_print: paged
...
We're going to install a python package called prodat, which will enable us to log and track our experiments through the power of *snapshots*.
...
This will be necessary so that prodat knows the proper directory to perform tracking in. 

```{r "setup", include=FALSE}
require("knitr")
opts_knit$set(root.dir = "~/Dev/prodat-R-example") # Replace with whatever your root directory for the project is
```

Now we're going to initialize a prodat repository. This will enable us to create snapshots for logging our experiments.
This only needs to be done once for a given repository.

```{r initializeprodat}
system("prodat init", input=c("my new project","test description"), timeout=15)
```
