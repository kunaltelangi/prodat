...
class TestLogger():
    def setup_class(self):
        ...
        if hasattr(prodatLogger, "instance"):
            del prodatLogger.instance
        prodatLogger(dirpath=self.temp_dir)

    def test_logger_basics(self):
        # Test basic logger functionality
        logger = prodatLogger.get_logger()
        random_text = str(uuid.uuid1())
        logger.warning(random_text)
        logger.error("TEST_ERROR")
        assert logger.level == prodatLogger(dirpath=self.temp_dir).logging_level
        
        # Test log files are created
        files = prodatLogger.get_logfiles()
        assert len(list(files)) > 0
        for f in files:
            assert prodatLogger(dirpath=self.temp_dir).logging_path in f
    ...
    def test_comprehensive_logger(self):
        ...
        f1 = prodatLogger.get_logger("foobar")
        f2 = prodatLogger.get_logger("foobar")
        assert f1 == f2, "Same-named loggers should be identical instances"
        
        # Test default log file creation
        default_logpath = os.path.join(self.temp_dir, ".prodat", "logs", "log.txt")
        ...
        l1 = prodatLogger.get_logger("logger1")
        l2 = prodatLogger.get_logger("logger2")
        ...
        custom_logger = prodatLogger.get_logger("custom_logger", random_name)
        ...
        custom_logpath = os.path.join(self.temp_dir, ".prodat", "logs", random_name)
        ...
        if prodatLogger().logging_level <= 10:  # DEBUG level
            @prodatLogger.timeit
            def slow_fn():
                sleep(0.1)  # Reduced sleep time for faster tests

            slow_fn()
            timer_logpath = os.path.join(self.temp_dir, ".prodat", "logs", "timers.log")
